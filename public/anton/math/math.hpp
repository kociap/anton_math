...
unsigned char _BitScanReverse64(unsigned long *_Index,
...
template <typename T> [[nodiscard]] constexpr T abs(T const a) {
  return a < T(0) ? -a : a;
}

template <typename T> [[nodiscard]] constexpr T max(T const a, T const b) {
  return a > b ? a : b;
}

template <typename T, typename... VT>
[[nodiscard]] constexpr T max(T const &arg1, T const &arg2, T const &arg3,
                              VT const &...args) {
...
template <typename T> [[nodiscard]] constexpr T min(T const a, T const b) {
  return a < b ? a : b;
}

template <typename T, typename... VT>
[[nodiscard]] constexpr T min(T const &arg1, T const &arg2, T const &arg3,
                              VT const &...args) {
  constexpr i64 array_size = sizeof...(VT) + 3;
  T const *arg_array[array_size] = {&arg1, &arg2, &arg3, &args...};
...
template <typename T>
[[nodiscard]] constexpr T clamp(T const x, T const lower_limit,
                                T const upper_limit) {
...
// lerp
// Computes the linear interpolation between a and b for the parameter t in the
// interval [0, 1].
//
...
// smootherstep
// Improved version of smoothstep that has zero 1st and 2nd order derivatives at
// egde0 and edge1.
//
// Returns 0 if x <= edge0, 1 if x >= edge1, otherwise computes 6x^5 - 15x^4 +
// 10x^3 with x rescaled to range [0.0, 1.0].
//
